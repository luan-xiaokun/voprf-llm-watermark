...
class BinaryClassifier(nn.Module):
    def __init__(
        self, input_dim: int, window_size: int, num_layers: int, hidden_dim: int = 64
    ):
...
def get_model(input_dim, window_size, model_path, layers=3):
...
def train_model(data_dir, bit_number, model_dir, window_size, layers):
    ...
    train_data = TensorDataset(
        torch.from_numpy(np.array(features)), torch.from_numpy(np.array(labels))
    )
    # Define a DataLoader
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data.json", help="data directory"
    )
    ...
    parser.add_argument(
        "--model_dir", type=str, default="model.pt", help="model directory"
    )
    args = parser.parse_args()
    train_model(
        args.data_dir, args.bit_number, args.model_dir, args.window_size, args.layers
    )
