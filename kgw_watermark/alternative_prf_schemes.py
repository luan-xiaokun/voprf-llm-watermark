...
def seeding_scheme_lookup(seeding_scheme: str):
    ...
    if seeding_scheme == "simple_1" or seeding_scheme == "lefthash":
        ...
        raise ValueError(
            f"Invalid seeding scheme name {seeding_scheme} given. Try  'simple_1'?"
        )
...
def anchored_skipgram_prf(
    input_ids: torch.LongTensor, salt_key: int, anchor: int = -1
) -> int:
    # maximum distance skipgram within context
    return (
        hashint(salt_key * input_ids[0]) * hashint(salt_key * input_ids[anchor])
    ).item()
...
def position_prf(input_ids: torch.LongTensor, salt_key: int, k: int = 2) -> int:
    return (
        (
            salt_key
            * input_ids
            * torch.arange(1, len(input_ids) + 1, device=input_ids.device)
        )
        .sum()
        .item()
    )
