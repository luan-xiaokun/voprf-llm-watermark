...
class HomoglyphCanonizer:
    ...
    def __call__(self, homoglyphed_str: str) -> str:
        ...
        homoglyph_table = self._select_canon_category_and_load(
...
class TrueCaser:
    ...
    def _spacy_truecasing(self, random_capitalized_string: str):
        ...
        truecased_str = "".join(
            [
                (
                    w.text_with_ws.capitalize()
                    if w.pos_ in POS or w.is_sent_start
                    else w.text_with_ws
                )
                for w in doc
            ]
    ...
    def _nltk_truecasing(self, random_capitalized_string: str):
        ...
        truecased_str = " ".join(
            [w.capitalize() if p in POS else w for (w, p) in tagged_text]
        )
        return truecased_str
