...
class TransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        )  # Ensure LSTM compatible shape
        output, _ = self.classifier(features)
        output = self.fc_hidden(
            output[:, -1, :]
        ...
        with open(filepath, "r") as f:
            for line in f:
                ...
                if llm_name == "gpt2":
                    ...
                    inputs = tokenizer(
                        inputs, return_tensors="pt", add_special_tokens=True
                    )
                ...
                inputs_bin = [
                    int_to_bin_list(n, bit) for n in inputs["input_ids"].squeeze()
                ]
...
def train_model(
    _bit_number, _input_dir, model_file, output_model_dir, b_layers, z_value, llm_name
):
    # Prepare data
    train_data = prepare_data(
        os.path.join(_input_dir, "train_data.jsonl"),
        train_or_test="train",
        bit=_bit_number,
        z_value=z_value,
        llm_name=llm_name,
    ...
    test_data = prepare_data(
        os.path.join(_input_dir, "test_data.jsonl"),
        train_or_test="test",
        bit=_bit_number,
        z_value=z_value,
        llm_name=llm_name,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output_model_dir", type=str, default="model/model_parameters4.pt"
    )
