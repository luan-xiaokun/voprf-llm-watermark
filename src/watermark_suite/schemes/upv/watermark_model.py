...
class WatermarkLogitsProcessor(LogitsProcessor):
    ...
    def __init__(
        self, vocab, delta, model, window_size, cache, bit_number, beam_size, llm_name
    ):
        self.vocab = vocab
        self.delta = delta
        self.model = model
    ...
    def _get_greenlist_ids(self, input_ids, scores):
        ...
        if self.mode == "sample":
            _, candidate_tokens = torch.topk(
                input=scores, k=20, largest=True, sorted=False
            )
        else:
...
class Watermark:
    ...
    def generate_list_with_green_ratio(self, length: int, green_ratio: float):
        ...
        token_list = (
            random.sample(self.vocab, self.window_size - 1)
            if self.window_size - 1 > 0
            else random.sample(self.vocab, 1)
        )
        is_green = []
    ...
    def generate_and_save_test_data(
        self, llm_name, dataset_name, output_dir, sampling_temp, max_new_tokens
    ):
        ...
        if llm_name == "gpt2":
            ...
            tokenizer = AutoTokenizer.from_pretrained(
                "facebook/opt-1.3b", use_fast=False
            )
            ...
            model = AutoModelForCausalLM.from_pretrained(
                "decapoda-research/llama-7b-hf", device_map="auto"
            )
        ...
        top_k = None
        if self.beam_size == 0:
            gen_kwargs.update(dict(do_sample=True, top_k=top_k, temperature=sampling_temp))
        else:
            gen_kwargs.update(dict(num_beams=self.beam_size))
        ...
        for line in lines:
            try:
                ...
                text = data["text"]
                text_tokenized = (
                    tokenizer(text, return_tensors="pt", add_special_tokens=True)
                ).to(device)
                prompt_length = 30
                ...
                prompt["attention_mask"] = text_tokenized["attention_mask"][
                    :, :prompt_length
                ]
                ...
                output_without_watermark = text_tokenized["input_ids"][
                    :, prompt_length : prompt_length + 200
                ]

                _, _, z_score = self.green_token_mask_and_stats(
                ...
                _, _, z_score = self.green_token_mask_and_stats(
                    output_without_watermark.squeeze(0)
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_dir", type=str, default="model/model_16_window_3_layer_5_new.pt"
    )
    ...
    watermark = Watermark(
        ...
        args.layers,
        delta=args.delta,
        model_dir=args.model_dir + "combine_model.pt",
        beam_size=args.n_beams,
    )
    watermark.generate_and_save_train_data(args.train_num_samples, args.output_dir)
    watermark.generate_and_save_test_data(
        args.llm_name,
        args.dataset_name,
        args.output_dir,
        args.sampling_temp,
