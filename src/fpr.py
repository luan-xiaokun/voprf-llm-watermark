...
def plot_fpr_curve(p_value_list: list[float], label: str, ax: plt.Axes):
    """
    从 p-value 列表绘制 经验FPR vs. 理论FPR 曲线。

    Args:
        p_value_list: 包含 p-value 的列表。
        label: 这条曲线的标签，用于图例。
        ax: 要在上面绘图的 matplotlib 坐标轴对象。
    """
    # 将列表转换为 numpy 数组以便进行高效计算
...
def main():
    ...
    for sample in dataset:
        ...
        # for i in range(0, len(token_ids) - STRIDE, STRIDE):
        #     chunk = token_ids[i : i + STRIDE]
        #     w4_results = [_local_detect(chunk, 4, gamma, server) for server in servers]
        #     w7_results = [_local_detect(chunk, 7, gamma, server) for server in servers]
    ...
    with open("fpr_4.npy", "wb") as f:
        np.save(f, array1)
    with open("fpr_7.npy", "wb") as f:
        np.save(f, array2)
